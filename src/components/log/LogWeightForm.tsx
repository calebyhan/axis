...
export function LogWeightForm({ onSave }: { onSave: () => void }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      if (kg) {
        setValue(resolvedUnits === "imperial" ? (kg * 2.20462).toFixed(1) : kg.toFixed(1));
      }
      setLoaded(true);
    });
  }, []);

  const step = units === "imperial" ? 0.5 : 0.1;
  ...
  async function handleSubmit() {
    ...
    const kg = units === "imperial" ? raw / 2.20462 : raw;
    const now = new Date();
  ...
  return (
    <div className="flex flex-col items-center gap-8 py-4">
      ...
      {/* Number display / inline editor */}
      <div className="flex items-center gap-6">
        <button
          type="button"
          onClick={() => nudge(-1)}
          className="w-12 h-12 flex items-center justify-center rounded-full border border-white/10 text-white/55 hover:text-white hover:border-white/20 active:scale-95 transition-all"
        >
          ...
          {editing ? (
            <input
              ...
              onBlur={() => {
                ...
                if (!isNaN(n) && n > 0) setValue(n.toFixed(1));
              }}
            ...
            <button
              ...
              className="text-5xl font-light tracking-tight text-white leading-none"
            >
              {value}
            </button>
          )}
          <span className="text-base text-white/40 mt-2">{weightUnit(units)}</span>
        </div>
